...
exports.run = function(client, message, args) {
  ...
  if (!message.guild) {
    return message.author.send('temizle komutu sadece sunucularda kullanýlabilir.');
  }
  let mesajsayisi = parseInt(args.join(' '));
  if (mesajsayisi.length < 1) return message.channel.send('Kaç mesaj silmem gerektiðini belirtmedin.')
